...
| A delete takes its keys in the body, capped at a thousand | `T27` of `docs/labelling-ui/plan.md` | Requirement 54 says the rows are ticked and the keys are taken; the shape is the plan's. A key a path segment would be N calls and no answer for a half-done group, and a repeated query parameter puts a thousand UUIDs in a URL. The cap is the read's own, for the same reason |
