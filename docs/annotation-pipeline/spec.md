...
71. **A record stores no answer space. The profile derives one.** Choosing the profile settles the answer *type* — an array of calls — and the record's own content settles the *space*: which names, and each name's `parameters`. Both are already on the record, so a stored answer space is a second copy of one of them, and the record carries no field for it.
    ...
    Measured, because the first draft of this requirement said the opposite and the numbers are what changed it. Deriving the catalog costs **0.27 µs** per record where the source carries it as data, against 0.07 µs to read a stored copy: **0.0 seconds across a 21,172-record run**, so the stored copy buys nothing on the shape this pipeline is for. Re-measured against the built code rather than a sketch of it: on the **declared input**, where `tools` is data, `catalog_names` costs **2.70 µs**, so the five callers a record has — three validity checks, `scenario_hash`, the capture control — spend **0.29 s** across a 21,172-record pass. That is the number this requirement rests on, and it supersedes the 0.27 µs above, which was measured before the reader built `Tool` objects.

Where a source renders its catalog into *prose* the parse is real and is paid per read: **119 µs**, so the same five callers spend **12.6 s** per pass. An earlier draft of this requirement claimed that paying it once at stage 0 "keeps that at zero too" — it does not, and the number is here instead of the claim. It is also not a cost of the input: the reference source is the one shaped that way, and it is the source this pipeline is verified against rather than built for. Two ways to remove it were rejected: a bounded parse cache, which buys 12 s in a pipeline whose jury pass costs hours, and normalising a legacy catalog into `meta.tools` at stage 0, which would add 2,888 bytes to every one of 21,172 rows — 61 MB on disk — to save the same 12 s. What the guard holds is that exactly one *module* parses, so the cost has one owner and stays measurable. Against which a stored copy costs a second thing that can disagree with the first, an artifact column on every record, and, for a compound answer, 3,247 bytes against the catalog's own 2,888 — a copy larger than the original. The pipeline's own rule already settled it: *a source's vocabulary is declared once, and everything derivable is derived.*
