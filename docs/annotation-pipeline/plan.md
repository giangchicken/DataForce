...
|---|---|
| `make check` | ruff, `mypy --strict` on `src/`, and 326 tests |
...
10. **Every name contains its result**, everywhere — not only on the contracts. `X_to_Y` for a conversion, `build_X` for something assembled, `read_X` for something pulled out of a larger structure; the convention is the corpus generator's own. Rejected: a single word naming an operation without its object (`adapt`, `parse`, `of`), a name that reads backwards (`label_of`), and any name that is also a stage name (`load`, `export`). — core § The two contracts, core Decisions
    ...
    **Revised again after T11: a name names one thing, and a joining word makes it a phrase.** `from`, `with`, `by`, `as` and the conjunctions proper describe a *relation between two things*, so the call site has to be read as a sentence rather than as a value. Measured across 153 functions: no conjunction anywhere, and thirteen prepositions. Five were phrases and are gone — `schema_for` → `artifact_schema`, `calls_by_name` → `named_calls`, `_with_provenance` → `_stamped_item`, `_raw_with_records` → `_raw_record_pairs`, `_turn_as_part` → `_turn_part`. The other eight stay, each for a reason the guard's allowlist carries: four are the `Y_from_X` form this decision itself established, two use `by reference`, which is the core spec's own term for media held by URI and checksum, one is the `dataclasses.asdict` idiom, and `label_not_in_catalog` is a declared identifier — a `params.yaml` key and a quarantine filename — where renaming the function renames a data contract. `test_naming.py` fails on a new joined name, on any of the five coming back, and on an allowlist entry whose function no longer exists, so the exemptions cannot outlive what they exempt.
