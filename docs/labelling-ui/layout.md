...
| control | what it does |
|---|---|
| `keep-table` *(tick per row)* | one row per **place** a claimed value stands: keep it or leave it, the field it is in, and the offsets into that field. What it stands in for is a box, retyped per occurrence and refused where it is not a placeholder of this value's kind. The value and its kind span its rows, because a value is one kind wherever it stands. What you untick stays in the text exactly as it arrived, struck through rather than dropped off the table |
...
| **Dataset** | **what the database holds right now**: which database it is, the counts, the statistics grid with its empty cells, every other distribution under it as bars ranked by count — never `domain` or `call_trigger`, which the grid already crosses — and a page of stored rows — the facets, not the samples — scrolling in a track of their own under a head that stays, so the sheet is not as long as the corpus. Narrowable to the rows nothing could validate. One row opens whole, saying every facet it was filed under — the columns and the keys in `notes`. **Only `tool_decision_dataset`, the redacted half** — there is no route to `tool_decision_record` and this sheet does not ask for one, because that table keeps what arrived un-redacted. Rows tick, and the ticked ones can be deleted — armed, then confirmed, out of both tables, leaving the queue row that imported them, answering no body so the line over the table says what was ticked | **Delete the selected** then **Yes, delete N rows**/**Cancel**, **Show more**, *only invalid*, *tick every row shown*, Close |
...
| `personal-data.js` | `adapter` | the values the reviewer keeps, the ones they add, and the copy that ships. **Not** where a value stands in the text — that is answered | `panel-data`, `keep-table`, `value-new`, `value-class`, `value-add`, `value-note`, `keep-note`, `kind-new`, `kind-add`, `kind-note`, `review-text`, `text-which`, `data-verdict`, `data-refusal` |
...
| `corpus.js` | `adapter` | what is already stored: the dataset sheet, the statistics grid, which rows a reviewer took back out, and which database a record lands in — said twice, once as the header's pill and once as the sheet's line, from one reading | `sheet-dataset`, `dataset-rows`, `dataset-bad`, `dataset-all`, `dataset-erase`, `dataset-keep`, `dataset-going`, `dataset-one`, `dataset-more`, `dataset-note`, `dataset-store`, `stats`, `strip`, `store` |
