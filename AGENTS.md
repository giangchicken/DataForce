...
- **R-6** `[review]` **Name a function with a verb phrase, verb first.** A function is a call and a call is an act, so the name says what it does: `count_rows`, never `row_counts`; `select_dataset_rows`, never `dataset_rows`. A noun tells a reader what came back and hides that anything happened, which is why it belongs to the variable instead. Never a leading underscore, and never an `of`/`for` suffix. The grammatical form is the signal, and this is one convention with `R-1` rather than two: the verb phrase is the act, the noun is the thing it answers with, `-er`/`-or` is what performs it — one stem per step, inflected, so the form alone says which side of the call a name sits on. **The noun is the one the return type already has.** Where something is declared for what comes back, the name is that declaration's: a function answering a `ToolDecisionSample` is `build_tool_decision_sample`, and one answering a `ToolDecisionDataset` would be a function that built the whole table — a name and a type disagreeing about how many is the reader's problem long before it is the compiler's. Where nothing is declared, the name still says what comes back; only a function that returns nothing is free to name the act alone. A route handler's act is the HTTP one, so it is named for that: `get_page`, `post_ai_review`. Three things are read rather than called and stay nouns: a property, a test fixture, and a constant. One thing keeps a name it did not choose: a stub, an override or a method answering someone else's interface is named after what it stands in for, because the caller is what decides.
...
- **R-8** `[review]` **The signature belongs to the unit, not to every function inside it.** A class, a module and a package are what a reader imports and holds in their head, so those carry the name of the thing they stand for — `ToolDecisionSampleBuilding`, `ToolDecisionSample` — and the functions under them do not have to repeat it: inside `profile/tool_decision/`, `count_by_facet` is already unambiguous and `count_tool_decision_samples_by_facet` is the same fact said twice. A function earns the name only where its own would otherwise be ambiguous at the call site it lands in. And the exception runs the other way for a layer that may **not** name the thing: there the family stem is whatever it is generic over, and the word its callers use is exactly the one that must not appear.
