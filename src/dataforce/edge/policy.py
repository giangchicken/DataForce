...
remembered: the digest and the value have one writer.
...
read as ``{}`` is an engine that holds no thresholds and says nothing about it, and a
declaration that is missing should stop the run before the first record. So the path is checked here,
