"""The LLM error taxonomy: twelve classes, preserved exactly.

Copied from ``agent-evaluation``'s ``src/components/llm/exceptions.py`` without
behavioral change. Every name, parent, and status code is load-bearing: the spec
pins this hierarchy so that ``except LLMRateLimitError`` and friends in
``agent-evaluation`` keep working when it swaps its local copy for this one.

``ToolkitError`` is deliberately *not* a parent here. A consumer catching
``LLMError`` is catching provider failures; a consumer catching ``ToolkitError``
is catching a malformed input file. Joining them would make one ``except`` clause
swallow the other's failures.
"""
