"""`parse_text_to_tools` pinned: what a model wrote, read back as calls.
...
The other direction of `convert_tools_to_text`, and the reason the panel needs no judge --
...
def test_a_call_is_read_as_the_format() -> None:
    ...
    assert parse_text_to_tools(
        '[{"name": "OpenTicket", "arguments": {"ma_khach": "KH-1"}}]'
    ) == (OPEN_TICKET,)


def test_the_wire_format_s_own_wrapper_is_the_same_call() -> None:
    """A juror may answer in the format it was shown; `convert_tools_to_text` is as lenient."""
    assert parse_text_to_tools(
...
def test_arguments_that_will_not_parse_stay_as_the_text() -> None:
    ...
    assert parse_text_to_tools('[{"name": "OpenTicket", "arguments": "KH-1"}]') == (
...
def test_an_answer_with_no_call_in_it_is_no_calls() -> None:
    ...
    """
    assert parse_text_to_tools("[]") == ()
    assert parse_text_to_tools("không cần gọi tool nào") == ()
