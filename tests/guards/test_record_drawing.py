...
`record.py` is the only place it is written next to the field. Two statements of one fact are
...
def test_the_scan_rejects_a_drawing_that_has_drifted(
    ...
    """Proved red: a key in the document with no field, a key dropped, and a nested key dropped."""
    assert differences(drift(drawn_record()), model_keys(Record)) != []


def test_the_scan_rejects_a_field_the_document_does_not_draw() -> None:
    """The other direction: `record.py` grows a key and § *The record* is not touched."""
