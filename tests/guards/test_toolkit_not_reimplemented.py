...
from defining one. A P31 pairing over two hand-maintained copies of a fact proves the copies match;
...
def test_the_scan_rejects_a_module_that_does_the_library_s_job(violation: str) -> None:
    """P29: proved red against a synthetic violation, one per root and one per shape of name.
...
def test_the_document_claims_nothing_the_library_does_not_own() -> None:
    """P31, in the one direction that is still the document's to get wrong.
