...
added without a row is invisible, and a row left behind after a rename points at nothing. A
...
**No exemption hatch.** An exemption annotates a line, and "this module has no row" has no line to annotate
...
def test_the_scan_rejects_a_module_with_no_row() -> None:
    """Proved red: the common one -- a module added and the drawing not touched."""
...
def test_the_scan_rejects_a_tree_that_has_drifted(mutation: dict[str, str]) -> None:
    """Proved red: a row that outlived its module, and a row reworded on one side only."""
